...
final readonly class Attachment
{
    ...
    public function __construct(
        ...
        private ?LanguageMap $description = null,
        ?IRL $fileUrl = null,
        ?string $content = null
    ) {
        if (!$fileUrl instanceof IRL && null === $content) {
    ...
    public function equals(Attachment $attachment): bool
    {
        ...
        if ($this->description instanceof LanguageMap xor $attachment->description instanceof LanguageMap) {
            return false;
        }

        if ($this->description instanceof LanguageMap && $attachment->description instanceof LanguageMap && !$this->description->equals($attachment->description)) {
            return false;
        }

        if ($this->fileUrl instanceof IRL xor $attachment->fileUrl instanceof IRL) {
            return false;
        }

        return !($this->fileUrl instanceof IRL && $attachment->fileUrl instanceof IRL && !$this->fileUrl->equals($attachment->fileUrl));
    }
}
